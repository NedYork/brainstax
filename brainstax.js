...
// instead of this, make ajax request for cards
// function readCardFile(file) {
//   var data = fs.readFileSync(file);
//   return JSON.parse(data);
// }
// like this.
getCardsOfDeck: function(deckId) {
  $.ajax({
    url: "/api/subjects/decks/" + deckId,
    type: "GET",
    success: function(cards) {
      ApiActions.addCards(cards);
    }
  });
}
...
function cardQuizCount() {
  ...
  for (var i=0; i<cards.length; i++) {
    var card = cards[i];
    if (card.interval === 0 || !card.interval) {
      count++;
    }
  }
  return count;
